...
class Grasp_Creator:
    ...
    def find_grasp(self, obj, fingers, obj_pos, obj_axis_angle, fixed_pos_and_roation = True):
        ...
        #print('args', args)
        #working 
        res =  so.fmin_slsqp(self.objective_func_4_finding_grasp, initial_guess, args = args, f_eqcons=self.eq_constraints_4_finding_grasp, f_ieqcons=self.ieq_intersecting_4_findeing_grasp,  iter=600, full_output= True) #, iprint=0, full_output= True) #, iprint=2 ) #f_ieqcons= self.ieq_constraints_4_finding_grasp, iprint=2)  
        #test
        ...
        dif = self.eq_constraints_4_finding_grasp(res[0], *args)
        print('intersection values', dif)
        ...
        for i in range(len(fingers)):
            ...
            while j < len(max_limits):
                ...
                if not finger.is_thumb() and j == n-2 and self.SHADOW_HAND_CALCULATION: #ensure that shadow hand has coupled joints.
                    bound[0] = min_limits[j+1]*2
                    bound[1] = max_limits[j+1]*2
                    j+=1
                bounds.append(bound)
                j+= 1
    ...
    def finger_config_from_solution(self, finger, x, start_idx):
        ...
        if finger.is_thumb() or not self.SHADOW_HAND_CALCULATION:
            ...
            #print('q',q)
            res_q = finger.translate_qest_to_q_joint(q)
            #print('res', res_q)   
            quv = np.append( res_q, x[start_idx + n: start_idx + n+2])
    ...
    def ieq_intersecting_4_findeing_grasp(self, x, *args):
        fingers, obj = args[:2]
        fingers_config, obj_pos, obj_angleaxis, obj_uv = self.result_devided_in_catagories(x, args)
        total_dist = []
        for i in range(len(fingers)): 
   ...
   #def just add finger, no uv is included
    def add_finger_to_grasp(self, finger, uv_o, obj, obj_rotation, obj_pos, fingers_in_grasp, fingers_in_graps_quv):
        ...
        quv,i = self.finger_config_from_solution(finger, initial_guess, 0)

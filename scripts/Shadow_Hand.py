...
class Shadow_hand:
    ...
    def __init__(self, mj_model, mj_data): 
        self.world_from_palm = self.world_from_palm_transformation(mj_model)
        self.palm_from_wold = np.linalg.inv(self.world_from_palm)
        self.fingers = self.create_fingers(mj_model, mj_data)
    ...
    def create_fingers(self,mj_model, mj_data):
        fingers = []
        
        fingers.append(Thumb(mj_model, mj_data, self.world_from_palm))
        fingers.append(F1(mj_model, mj_data, self.world_from_palm))
        fingers.append(F2(mj_model, mj_data, self.world_from_palm))
        fingers.append(F3(mj_model, mj_data, self.world_from_palm))
        
        fingers.append(LF(mj_model, mj_data, self.world_from_palm))
       
        return fingers

    def world_from_palm(self):
        trans = worl
...
class Finger: 
    ...
    def __init__(self, mj_model, mj_data, world_from_palm):
   ...
   #PARTH_2_JSON = '/home/klinkby/Documents/Mujoco_Py_Workspace/Finger_Tip_Manipulation_Shadow_Hand/Shadow_Hand_21_03_2024/assets/th_distal_pst.json'
    ...
    def front_mesh_vectors(self, total_mesh):
        ...
        for i in range(0, l):
            ...
            add = True
            for p in triangle:
                if (abs(p[2]) <2 and  abs(p[1]) < 2 ): 
                    add = False
            if add == True:
                n = total_mesh.normals[i]
                if n[0] <= 0 and  n[2] >= 0 : 
...
class MF(Finger):
    ...
    UV_limit = [(0.24,2.9),(1.25769, 1.33309+  1.52037)]
    ...
    def front_mesh_vectors(self, total_mesh):
        ...
        for i in range(0, l):
            triangle= total_mesh.vectors[i]
            add = True
            for p in triangle:   #fjernere firkant i midten
                if (abs(p[2]) <2 and  abs(p[1]) < 2 ): 
                    add = False
            if add == True:
                n = total_mesh.normals[i]
                if n[1] <= 0 and  n[2] >= 0 and triangle[0][1] < 3: 
                    front_mesh_vectors.append(triangle) #self.scale_triangle(triangle))
        return front_mesh_vectors
    ...
    def quv_joint_from_acts(self, quv, do_shadow_hand_calculation = False):
        if not do_shadow_hand_calculation:
            ...
            for q in quv[:-3]:
                new_conf.append(q)
            q0_q1 = quv[-3] 
            max_limit = self.max_limits[-4]
            if q0_q1 <= max_limit: 
                new_conf.append(q0_q1)
                new_conf.append(0)
            else: 
                new_conf.append(max_limit)      
                new_conf.append(q0_q1-max_limit)

...
class Arc:
    ...
    def create_arc(self, start_node, target_node, time_to_reach_goal=1, test_existence= False):
        ...
        for j in range(0,number_steps):
            ...
            #update
            new_obj_uv = self.update_contacts(h,  obj_pos, obj_uv, roll_direction_and_velocity, curvatures_obj, obj_rotation)
            #new_obj_uv1 = self.update_contacts_from_finger(obj_pos, obj_rotation, fingers, quv_estimates)
            #print(new_obj_uv, new_obj_uv1)
    ...
    def quv_error_adjust(self, quv_estimates, obj_uv, obj_pos, obj_rotation, finger_acts_sorted):
        ...
        for i in range(len(finger_acts_sorted)):
            ...
            if rank == 5:
                ...
                q_displ = np.matmul(inv_J, pos_and_rot_displ) 
                print(q_displ)
                new_quv =  quv + np.pad(q_displ, [(0, 2)], mode='constant', constant_values=0)
                #print('use direct method', q_displ)
                #q_displ = self.five_joint_fingers_quv_adjustment([quv], [obj_uv[i]], obj_pos, obj_rotation, 0,[f])[0] 
                #new_quv = quv + q_displ
                
            else: 
                
                #print('use optimizer fist try with out uv')
                #new_quv = self.quv_adjust_using_optimizer(f, quv, obj_rotation, obj_pos)
